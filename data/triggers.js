...
module.exports = function(a) {
  ...
  app.get('triggerHandler').addTrigger(
    new Trigger({
      title: 'imagine',
      match: function(message) {
        var regex = /You feel your trance deepening to a deep level/i;
        if ((match = message.match(regex))) {
          return match;
        }
      },
      success: function(results) {
        app.get('actionClient').publish('actions', 'imagine jorge');
        app.get('actionClient').publish('actions', 'te all');
      },
    })
  );
        ...
        try {
          ...
          if (knownMessages.indexOf(results[2]) != -1) {
            ...
            if (!app.get('didGuess') && results[1] == 2) {
              console.log(
                `I didn't guess on 1 and don't know no ${results[1]} so I'm just guessing.`
              );
